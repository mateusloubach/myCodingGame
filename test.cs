...
class Program
{
    static void Main()
    {
        ...
        {
            // Read input
            string[] podInput = Console.ReadLine().Split();
            int x = int.Parse(podInput[0]);
            int y = int.Parse(podInput[1]);
            int nextCheckpointX = int.Parse(podInput[2]);
            int nextCheckpointY = int.Parse(podInput[3]);
            int nextCheckpointDist = int.Parse(podInput[4]);
            int nextCheckpointAngle = int.Parse(podInput[5]);

            string[] opponentInput = Console.ReadLine().Split();
            int opponentX = int.Parse(opponentInput[0]);
            int opponentY = int.Parse(opponentInput[1]);

            // Implement your strategy here

            // Example: Move towards the next checkpoint with adjusted thrust based on angle difference
            Tuple<int, int, int> move = MoveTowardsTarget(x, y, nextCheckpointX, nextCheckpointY, nextCheckpointAngle);
            
            // Output your move
            Console.WriteLine($"{move.Item1} {move.Item2} {move.Item3}");
        }
    }

    static Tuple<int, int, int> MoveTowardsTarget(int currentX, int currentY, int targetX, int targetY, int nextCheckpointAngle)
    {
        // Calculate the angle difference threshold for thrust adjustment
        int angleThreshold = 45;

        // Calculate distance to the target
        double distance = Math.Sqrt(Math.Pow(targetX - currentX, 2) + Math.Pow(targetY - currentY, 2));

        // Calculate the adjusted thrust based on angle difference
        int thrust = (Math.Abs(nextCheckpointAngle) > angleThreshold) ? 0 : (int)(100 * (1 - Math.Abs(nextCheckpointAngle) / angleThreshold));

        // Move towards the target with the adjusted thrust
        double angle = Math.Atan2(targetY - currentY, targetX - currentX);
        return Tuple.Create((int)targetX, (int)targetY, thrust);
    }
}